...
class StreamlitAppSmokeTests(unittest.TestCase):
    def test_main_openai_compatible_app_renders_and_handles_local_fallback_chat(self) -> None:
        ...
        app.chat_input[0].set_value("oi").run(timeout=20)
        ...
        self.assertEqual(messages[-2]["content"], "oi")
        self.assertEqual(messages[-1]["role"], "assistant")
        self.assertIn("OPENAI_API_KEY", messages[-1]["content"])

    def test_main_qwen_renders_core_tabs_and_operational_controls(self) -> None:
        app = AppTest.from_file("main_qwen.py")
        app.run(timeout=20)
        ...
        self.assertIn("Executar comparação entre modelos", button_labels)
        ...
        self.assertIn("Provider de geração", select_labels)
